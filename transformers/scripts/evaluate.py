    ...
    with torch.no_grad():
        ...
        if model.rnn_type == "lstm":
            if not isinstance(hidden, tuple):
                hidden = (hidden, torch.zeros_like(hidden[0]))
...
def evaluate_model(config_path):
    ...
    encoder = Encoder(vocab_size, model_cfg["emb_dim"], model_cfg["hidden_dim"],
                      model_cfg["n_layers"], model_cfg["dropout"], pad_idx, model_cfg["rnn_type"])
    decoder = Decoder(vocab_size, model_cfg["emb_dim"], model_cfg["hidden_dim"],
                      model_cfg["n_layers"], model_cfg["dropout"], pad_idx, model_cfg["rnn_type"])
    model = Seq2Seq(encoder, decoder, device, sos_idx, eos_idx, model_cfg["rnn_type"]).to(device)
